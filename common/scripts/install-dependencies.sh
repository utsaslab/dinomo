...
if [ "$COMPILER" = "clang++" ] && [ -z "$(command -v clang++)" ]; then
  ...
  apt-add-repository "deb http://apt.llvm.org/trusty/ llvm-toolchain-trusty-5.0 main" > /dev/null
  apt-get install -y --force-yes clang-5.0 lldb-5.0 clang-format-5.0 > /dev/null
  update-alternatives --install /usr/bin/clang clang /usr/bin/clang-5.0 1 > /dev/null
  update-alternatives --install /usr/bin/clang++ clang++ /usr/bin/clang++-5.0 1 > /dev/null
  update-alternatives --install /usr/bin/clang-format clang-format /usr/bin/clang-format-5.0 1 > /dev/null
fi

if [ "$COMPILER" = "g++" ] && [ -z "$(command -v g++)" ]; then
  echo "Installing g++..."
  $PKG_MGR install -y gcc g++ > /dev/null

  if [ "$DIST" = "fedora" ]; then
    $PKG_MGR install -y gcc-c++ > /dev/null
  fi
fi
...
if [ "$DIST" = "debian" ]; then
  ...
  $PKG_MGR install -y build-essential autoconf automake libtool unzip pkg-config wget make libc++-dev libc++abi-dev > /dev/null
elif [ "$DIST" = "fedora" ]; then
  echo -e "Installing the following packages via $PKG_MGR:\n\t* autoconf\n\t* automake\n\t* libtool\n\t* build-essential \n\t* make"
  $PKG_MGR install -y build-essential autoconf automake libtool make > /dev/null
else
  exit 1
fi
...
if [ -z "$(command -v cmake)" ]; then
  ...
  mkdir /usr/cmake
  mv cmake-3.11.4-Linux-x86_64/* /usr/cmake/
