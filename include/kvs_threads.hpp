...
const unsigned kServerReplicationChangePort = 6700;
...
const unsigned kCacheIpResponsePort = 6800;
...
const unsigned kRoutingReplicationResponsePort = 7200;
...
const unsigned kDepartDonePort = 7600;

// The port on which the monitoring nodes listens for performance feedback from
// clients.
//const unsigned kFeedbackReportPort = 6750;
const unsigned kFeedbackReportPort = 7700;

// The port on which the monitoring nodes listens for cluster-internal stat
// feedback from memory nodes.
//const unsigned kInternalStatReportPort = 6800;
const unsigned kInternalStatReportPort = 7800;

// The port on which storage threads listen for join responses
//const unsigned kServerResponsePort = 6850;
const unsigned kServerResponsePort = 7900;

// The port on which benchmark nodes listen for triggers.
//const unsigned kBenchmarkCommandPort = 6900;
const unsigned kBenchmarkCommandPort = 8000;

// The port on which storage nodes retrieve their restart counts from the
// management system.
//const unsigned kKopsRestartCountPort = 7000;
const unsigned kKopsRestartCountPort = 8100;

// The port on which the management server will listen for requests for
// executor nodes.
//const unsigned kKopsFuncNodesPort = 7002;
const unsigned kKopsFuncNodesPort = 8102;
...
public:
  ...
  Address failover_connect_address() const {
    return private_base_ + std::to_string(tid_ + kFailoverPort);
  }

  Address failover_bind_address() const {
    return kBindBase + std::to_string(tid_ + kFailoverPort);
  }
  ...
  Address response_connect_address() const {
    return private_base_ + std::to_string(tid_ + kServerResponsePort);
  }

  Address response_bind_address() const {
    return kBindBase + std::to_string(tid_ + kServerResponsePort);
  }
#endif
};
...
#ifdef ENABLE_DINOMO_KVS
  Address internal_stat_report_connect_address() const {
    return ip_base_ + std::to_string(kInternalStatReportPort);
  }

  Address internal_stat_report_bind_address() const {
    return kBindBase + std::to_string(kInternalStatReportPort);
  }
#endif
};
