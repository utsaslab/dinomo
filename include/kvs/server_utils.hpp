...
#define DATA_REDISTRIBUTE_THRESHOLD 10000
...
typedef DinomoKVStore<sekwonlee::AdaptiveHybridCache<uint64_t, char *>> DinomoAHCKVS;
typedef DinomoKVStore<sekwonlee::HybridCache<uint64_t, char *>> DinomoHCKVS;
typedef DinomoKVStore<sekwonlee::EmptyCache<uint64_t, char *>> DinomoECKVS;
#else
typedef CloverKVStore CloverKVS;
#endif
#else
...
typedef KVStore<Key, SingleKeyCausalLattice<SetLattice<string> > > MemorySingleKeyCausalKVS;
typedef KVStore<Key, MultiKeyCausalLattice<SetLattice<string> > > MemoryMultiKeyCausalKVS;
...
class Serializer
{
    public:
        ...
        virtual string getMeta(const Key &key, AnnaError &error) = 0;
        virtual unsigned putMeta(const Key &key, const string &serialized) = 0;
        virtual string getReplicated(const Key &key, AnnaError &error) = 0;
        virtual unsigned putReplicated(const Key &key, const string &serialized) = 0;
        virtual unsigned update(const Key &key, const string &serialized) = 0;
        virtual unsigned updateReplicated(const Key &key, const string &serialized) = 0;
        virtual void merge(bool clear_cache) = 0;
        virtual void flush() = 0;
        virtual void failover(int failed_node_rank) = 0;
        virtual void invalidate(const Key &key) = 0;
        virtual void swap(const Key &key, bool replicated) = 0;
#endif
};
...
class MemoryLWWSerializer : public Serializer
{
    ...
    public:
    MemoryLWWSerializer(T *kvs) : kvs_(kvs) {}
    ...
    string get(const Key &key, AnnaError &error)
    {
        std::pair<char *, size_t> ret = kvs_->get(stoul(key, nullptr, 0), sizeof(uint64_t), error);
        LWWPairLattice<string> val = deserialize_lww(string(ret.first, ret.second));
        if (val.reveal().value == "")
        {
            error = AnnaError::KEY_DNE;
        }
    ...
    {
        LWWPairLattice<string> val = deserialize_lww(serialized);
        // TODO: need to add a function removing alphabetic representations from the key.
        return kvs_->put(stoul(key, nullptr, 0), sizeof(uint64_t), serialized.c_str(), serialized.size() + 1);
        //return val.reveal().size();
    }
    
    void remove(const Key &key) { kvs_->remove(stoul(key, nullptr, 0), sizeof(uint64_t)); }

    string getMeta(const Key &key, AnnaError &error)
    {
        std::pair<char *, size_t> ret = kvs_->getMeta(key.c_str(), key.size() + 1, error);
        LWWPairLattice<string> val = deserialize_lww(string(ret.first, ret.second));
        if (val.reveal().value == "")
        {
            error = AnnaError::KEY_DNE;
    ...
    unsigned putMeta(const Key &key, const string &serialized)
    {
        LWWPairLattice<string> val = deserialize_lww(serialized);
        kvs_->putMeta(key.c_str(), key.size() + 1, serialized.c_str(), serialized.size() + 1);
        return val.reveal().size();
    }

    string getReplicated(const Key &key, AnnaError &error)
    {
        std::pair<char *, size_t> ret = kvs_->getReplicated(stoul(key, nullptr, 0), sizeof(uint64_t), error);
        LWWPairLattice<string> val = deserialize_lww(string(ret.first, ret.second));
        if (val.reveal().value == "")
        {
            error = AnnaError::KEY_DNE;
    ...
    unsigned putReplicated(const Key &key, const string &serialized)
    {
        LWWPairLattice<string> val = deserialize_lww(serialized);
        kvs_->putReplicated(stoul(key, nullptr, 0), sizeof(uint64_t), serialized.c_str(), serialized.size() + 1);
        return val.reveal().size();
    }

    unsigned update(const Key &key, const string &serialized)
    {
        LWWPairLattice<string> val = deserialize_lww(serialized);
        return kvs_->update(stoul(key, nullptr, 0), sizeof(uint64_t), serialized.c_str(), serialized.size() + 1);
        //return val.reveal().size();
    }

    unsigned updateReplicated(const Key &key, const string &serialized)
    {
        LWWPairLattice<string> val = deserialize_lww(serialized);
        kvs_->updateReplicated(stoul(key, nullptr, 0), sizeof(uint64_t), serialized.c_str(), serialized.size() + 1);
        return val.reveal().size();
    }

    void merge(bool clear_cache) { kvs_->merge(clear_cache); }

    void flush() { kvs_->flush(); }

    void failover(int failed_node_rank) { kvs_->failover(failed_node_rank); }

    void invalidate(const Key &key) { kvs_->invalidate(stoul(key, nullptr, 0)); }

    void swap(const Key &key, bool replicated) { kvs_->swap(stoul(key, nullptr, 0), replicated); }
#endif
};
...
class MemorySetSerializer : public Serializer
{
    ...
    void remove(const Key &key) { kvs_->remove(key); }
#ifdef ENABLE_DINOMO_KVS
    string getMeta(const Key UNUSED(&key), AnnaError UNUSED(&error)) { return string(); }

    unsigned putMeta(const Key UNUSED(&key), const string UNUSED(&serialized)) { return 1; }

    string getReplicated(const Key UNUSED(&key), AnnaError UNUSED(&error)) { return string(); }

    unsigned putReplicated(const Key UNUSED(&key), const string UNUSED(&serialized)) { return 1; }

    unsigned update(const Key UNUSED(&key), const string UNUSED(&serialized)) { return 1; }

    unsigned updateReplicated(const Key UNUSED(&key), const string UNUSED(&serialized)) { return 1; }
    ...
    void invalidate(const Key UNUSED(&key)) {}

    void swap(const Key UNUSED(&key), bool UNUSED(replicated)) {}
#endif
};
...
class DiskLWWSerializer : public Serializer
{
    ...
    unsigned put(const Key &key, const string &serialized)
    {
        ...
        if (!input)
        { // in this case, this key has never been seen before, so we
            ...
            return output.tellp();
                    ...
                    &input))
        { // if we have seen the key before, attempt to
...
class DiskPrioritySerializer : public Serializer
{
    ...
    unsigned put(const Key &key, const string &serialized) override
    {
        ...
        int fd = open(fname(key).c_str(), O_RDWR | O_CREAT, 666);
        if (fd == -1)
        {
