...
public:
  ...
  ServerThreadList get_partition_range_changed_threads(Address public_ip, Address private_ip, unsigned tid) {
    ServerThreadList threads;

    for (unsigned virtual_num = 0; virtual_num < kVirtualThreadNum; virtual_num++)
    {
        ServerThread st = ServerThread(public_ip, private_ip, tid, virtual_num);
        auto pos = ConsistentHashMap<ServerThread, H>::find_predecessor(st);
        if (std::find(threads.begin(), threads.end(), pos->second) == threads.end())
        {
...
public:
  ...
  void issue_replication_factor_local_request(const Address &respond_address,
                                        const Key &key,
                                        SocketCache &pushers, const Address &target_address);
};
