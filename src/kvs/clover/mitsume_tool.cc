...
int mitsume_tool_local_hashtable_operations(
  ...
  struct mitsume_hash_struct *search_hash_ptr, *search_old_hash_ptr;
                            ...
                            // required hashtable
  {
    if (operation_flag == MITSUME_CHECK_OR_ADD ||
      ...
      if (MITSUME_TOOL_SHARE_FIFO_QUEUE[lru_target].size() >
        ...
        search_old_hash_ptr = MITSUME_TOOL_QUERY_HASHTABLE[old_bucket][old_key];
        MITSUME_TOOL_QUERY_HASHTABLE[old_bucket].erase(old_key);
        mitsume_tool_cache_free(search_old_hash_ptr, MITSUME_ALLOCTYPE_HASH_STRUCT);
        if (old_bucket != bucket)
...
int mitsume_tool_query_read(struct mitsume_consumer_metadata *thread_metadata,
                       mitsume_key key,
                       struct mitsume_tool_communication *input,
                       int option_flag, int debug_flag, bool &cacheHit) {
  ...
  */
  ...
  int coro_id = thread_metadata->thread_id;
...
#ifdef CLOVER_DEBUG
  if (found)
      local_cache_hit_counter.fetch_add(1);
  else
      local_cache_miss_counter.fetch_add(1);
#endif
  ...
  if (option_flag & MITSUME_TOOL_QUERY_FORCE_REMOTE || !found) {
    ...
    mitsume_send_full_message(
        ...
        target_controller_id, sizeof(mitsume_msg), MITSUME_ENTRY_QUERY);
...
// yield, int coro_id)
{
  ...
  query_ret = mitsume_tool_query_read(thread_metadata, key, &query, 0, 0, cacheHit);
...
//                           test_yield);
  return mitsume_tool_open(thread_metadata, key, write_addr, size,
                           replication_factor, thread_metadata->thread_id,
                           test_yield);
}
int mitsume_tool_open(struct mitsume_consumer_metadata *thread_metadata,
                      mitsume_key key, void *write_addr, uint32_t size,
...
*/
  ...
  if (query_ret == MITSUME_SUCCESS) // hit local cache
  {
...
// finish step0
  ...
  mitsume_send_full_message(
      ...
      sizeof(mitsume_msg), MITSUME_ENTRY_OPEN);
  // send to controller
...
int mitsume_tool_write(struct mitsume_consumer_metadata *thread_metadata,
                       ...
                       coro_yield_t &yield, bool &cacheHit) {
  /*
  ...
  */
  ...
  query_ret = mitsume_tool_query(thread_metadata, key, &query, 0, 0, cacheHit);
