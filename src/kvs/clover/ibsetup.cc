...
#ifdef CLOVER_DEBUG
std::atomic<uint64_t> rdma_write_rtts;
std::atomic<uint64_t> rdma_cas_rtts;
std::atomic<uint64_t> rdma_read_rtts;
std::atomic<uint64_t> rdma_send_alloc_rtts;
std::atomic<uint64_t> rdma_send_insert_rtts;
std::atomic<uint64_t> rdma_send_fetch_rtts;
std::atomic<uint64_t> rdma_send_gc_rtts;

std::atomic<uint64_t> rdma_write_latency;
std::atomic<uint64_t> rdma_cas_latency;
std::atomic<uint64_t> rdma_read_latency;
std::atomic<uint64_t> rdma_send_alloc_latency;
std::atomic<uint64_t> rdma_send_insert_latency;
std::atomic<uint64_t> rdma_send_fetch_latency;
std::atomic<uint64_t> rdma_send_gc_latency;

std::atomic<uint64_t> rdma_write_payload;
std::atomic<uint64_t> rdma_cas_payload;
std::atomic<uint64_t> rdma_read_payload;
std::atomic<uint64_t> rdma_send_alloc_payload;
std::atomic<uint64_t> rdma_send_insert_payload;
std::atomic<uint64_t> rdma_send_fetch_payload;
std::atomic<uint64_t> rdma_send_gc_payload;

std::atomic<uint64_t> local_cache_hit_counter;
std::atomic<uint64_t> local_cache_miss_counter;
#endif
...
struct ib_inf *ib_complete_setup(struct configuration_params *input_arg,
  ...
  // post all rc qps
  // TODO: Publish local rcqps for this compute node
  ...
  // TODO: Only wait and receive rcqps from initial compute nodes.
  ...
  // TODO: Establish connection between initial machines only
  total_qp_count = 0;
  ...
  total_qp_count = 0;
  // post all ud qp
  // TODO: Publish local udqps for this compute node
  ...
  // TODO: Only wait and receive udqps from initial compute nodes.
  for (cumulative_id = 0; cumulative_id < total_machines; cumulative_id++) {
    for (i = 0; i < node_share_inf->num_local_udqps; i++) {
      char srv_name[RSEC_MAX_QP_NAME];
...
int userspace_one_read(struct ib_inf *ib_ctx, uint64_t wr_id,
...
#ifdef CLOVER_DEBUG
    rdma_read_rtts.fetch_add(1);
    rdma_read_payload.fetch_add((uint64_t)request_size);
#ifdef LATENCY_DEBUG
  ...
  rdma_read_latency.fetch_add(duration.count());
#endif
#endif
  return 0;
}
...
int userspace_one_cs(struct ib_inf *ib_ctx, uint64_t wr_id,
...
#ifdef CLOVER_DEBUG
    rdma_cas_rtts.fetch_add(1);
    rdma_cas_payload.fetch_add(sizeof(uint64_t));
#ifdef LATENCY_DEBUG
    auto starttime = std::chrono::system_clock::now();
#endif
#endif
  ...
  auto duration = std::chrono::duration_cast<std::chrono::nanoseconds> (
          std::chrono::system_clock::now() - starttime);
  rdma_cas_latency.fetch_add(duration.count());
#endif
#endif
  return 0;
}
...
int userspace_one_write(struct ib_inf *ib_ctx, uint64_t wr_id,
                        ...
                        unsigned long long offset) {
#ifdef CLOVER_DEBUG
    rdma_write_rtts.fetch_add(1);
    rdma_write_payload.fetch_add((uint64_t)request_size);
#ifdef LATENCY_DEBUG
    auto starttime = std::chrono::system_clock::now();
#endif
#endif
  ...
  auto duration = std::chrono::duration_cast<std::chrono::nanoseconds> (
          std::chrono::system_clock::now() - starttime);
  rdma_write_latency.fetch_add(duration.count());
#endif
#endif
  return 0;
}
...
#endif
  for (int i = 0; i < input_sge_length; i++)
    rdma_write_payload.fetch_add((uint64_t)input_sge[i].length);
#endif
  return 0;
}
...
#endif
  for (int i = 0; i < input_sge_length; i++)
      rdma_read_payload.fetch_add((uint64_t)input_sge[i].length);
#endif
  return 0;
}
...
int userspace_one_poll_once(struct ibv_cq *cq, int tar_mem,
        struct ibv_wc *input_wc, int sleep_flag)
{
    return ibv_poll_cq(cq, tar_mem, &input_wc[0]);
}
...
int mitsume_send_full_message(struct ib_inf *ib_ctx,
                              ...
                              uint32_t send_size, uint32_t type) {
  // verify send message
  ...
  auto starttime = std::chrono::system_clock::now();
#endif
  if (header_ptr->type == MITSUME_CONTROLLER_ASK_ENTRY) {
      rdma_send_alloc_rtts.fetch_add(1);
      rdma_send_alloc_payload.fetch_add((uint64_t)send_size);
  } else if (header_ptr->type == MITSUME_ENTRY_REQUEST) {
      if (type == MITSUME_ENTRY_OPEN) {
          rdma_send_insert_rtts.fetch_add(1);
          rdma_send_insert_payload.fetch_add((uint64_t)send_size);
      } else if (type == MITSUME_ENTRY_QUERY) {
          rdma_send_fetch_rtts.fetch_add(1);
          rdma_send_fetch_payload.fetch_add((uint64_t)send_size);
      }
  } else if (header_ptr->type == MITSUME_GC_REQUEST || header_ptr->type == MITSUME_GC_REQUEST_ACK ||
          header_ptr->type == MITSUME_GC_EPOCH_FORWARD || header_ptr->type == MITSUME_GC_EPOCH_FORWARD_ACK ||
          header_ptr->type == MITSUME_GC_RECYCLE_REQUEST || header_ptr->type == MITSUME_GC_RECYCLE_REQUEST_ACK ||
          header_ptr->type == MITSUME_GC_BACKUP_UPDATE_REQUEST || header_ptr->type == MITSUME_GC_BACKUP_UPDATE_REQUEST_ACK) {
      rdma_send_gc_rtts.fetch_add(1);
      rdma_send_gc_payload.fetch_add((uint64_t)send_size);
  }
#endif
  ...
  auto duration = std::chrono::duration_cast<std::chrono::nanoseconds> (
          std::chrono::system_clock::now() - starttime);
  if (header_ptr->type == MITSUME_CONTROLLER_ASK_ENTRY) {
      rdma_send_alloc_latency.fetch_add(duration.count());
  } else if (header_ptr->type == MITSUME_ENTRY_REQUEST) {
      if (type == MITSUME_ENTRY_OPEN) {
          rdma_send_insert_latency.fetch_add(duration.count());
      } else if (type == MITSUME_ENTRY_QUERY) {
          rdma_send_fetch_latency.fetch_add(duration.count());
      }
  } else if (header_ptr->type == MITSUME_GC_REQUEST || header_ptr->type == MITSUME_GC_REQUEST_ACK ||
          header_ptr->type == MITSUME_GC_EPOCH_FORWARD || header_ptr->type == MITSUME_GC_EPOCH_FORWARD_ACK ||
          header_ptr->type == MITSUME_GC_RECYCLE_REQUEST || header_ptr->type == MITSUME_GC_RECYCLE_REQUEST_ACK ||
          header_ptr->type == MITSUME_GC_BACKUP_UPDATE_REQUEST || header_ptr->type == MITSUME_GC_BACKUP_UPDATE_REQUEST_ACK) {
      rdma_send_gc_latency.fetch_add(duration.count());
  }
#endif
#endif
