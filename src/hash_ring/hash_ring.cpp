        ...
        const vector<Tier> &tiers, bool &succeed, unsigned &seed)
{
    ...
    else
    {
        ...
        if (key_replication_map.find(key) == key_replication_map.end())
        {
            ...
            return get_responsible_threads(response_address, key, metadata,
                    global_hash_rings, local_hash_rings, key_replication_map,
                    pushers, tiers, succeed, seed);
#else
...
void HashRingUtilInterface::issue_replication_factor_local_request(
        const Address &response_address, const Key &key,
