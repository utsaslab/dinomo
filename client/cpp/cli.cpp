...
void handle_request(KvsClientInterface *client, string input)
{
    vector<string> v;
    split(input, ' ', v);
    v.push_back(string(FIXED_VALUE_SIZE, '*'));
    ...
    else if (v[0] == "UPDATE")
    {
        ...
        string rid = client->update_async(key, serialize(val), LatticeType::LWW);
        vector<KeyResponse> responses = client->receive_async();
        while (responses.size() == 0)
        {
            responses = client->receive_async();
    ...
    else
    {
        std::cout << "Unrecognized command " << v[0]
                  << ". Valid commands are GET/READ, GET_SET, PUT/INSERT, UPDATE, PUT_SET, PUT_CAUSAL, "
